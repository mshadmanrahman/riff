...
(() => {
  ...
  const extractFeedPostAuthor = (postEl) => {
    ...
    if (!name) {
      ...
      if (anyProfileLink && !anyProfileLink.closest('[data-component-type="LazyColumn"]')) {
        ...
        if (slugMatch) {
          const slug = slugMatch[1].replace(/-[a-f0-9]{6,}$/, ""); // strip trailing hash
      ...
      if (sidebar) {
        ...
        if (sidebarLink) {
          ...
          if (!name) {
            ...
            const slugMatch = href.match(/\/in\/([^/?]+)/);
            if (slugMatch) {
              const slug = slugMatch[1].replace(/-[a-f0-9]{6,}$/, "");
              name = slug.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
            }
          }
        }
  ...
  const VIDEO_SETTINGS_PATTERN = /TextColor|Caption Area|Font Size|Text Edge|Font Family|Proportional Sans/i;
  ...
  const extractFeedPostText = (postEl) => {
    ...
    // Remove video/media player elements (contains caption settings UI)
    clone.querySelectorAll("video, track, [data-testid*='video']").forEach(el => {
      // Walk up to the video container and remove it
      let container = el.parentElement;
      for (let i = 0; i < 3 && container; i++) {
        if (cleanText(container.textContent).length > 100 &&
            VIDEO_SETTINGS_PATTERN.test(container.textContent)) {
          container.remove();
          break;
        }
        container = container.parentElement;
    ...
    // Also directly remove any element containing video settings text
    clone.querySelectorAll("div, span").forEach(el => {
      if (VIDEO_SETTINGS_PATTERN.test(el.textContent) && el.children.length < 3) {
        el.remove();
      }
    });
    ...
    // Remove small UI fragments
    clone.querySelectorAll('a[href*="follow"], span[aria-hidden="true"]').forEach(el => {
    ...
    for (const el of allEls) {
      ...
      if (VIDEO_SETTINGS_PATTERN.test(text)) continue;
  ...
  const isFeedMyPost = (postEl) => {
    // On the feed, your own posts typically don't have a Follow button
    const hasFollowBtn = postEl.querySelector('button[aria-label*="Follow"]');
    if (!hasFollowBtn) {
      // Check if there's a profile link that looks like yours
      // Also check for "Edit" or author-specific actions
      const editBtn = postEl.querySelector('button[aria-label*="Edit"], button[aria-label*="edit post"]');
      if (editBtn) return true;

      // If no Follow button AND the post has content, it's likely yours
      // But this is heuristic — we might be wrong for company pages, etc.
      // For now, absence of Follow = likely your post
      return true;
    }
    return false;
  };
  ...
  const extractFeedComments = (postEl) => {
    ...
    for (const [block, firstLink] of commentBlocks) {
      ...
      // Strategy C: extract from href: /in/first-last → "first last"
      if (!authorName && blockProfileLinks.length > 0) {
        const href = blockProfileLinks[0].getAttribute("href") || "";
        const slugMatch = href.match(/\/in\/([^/?]+)/);
        if (slugMatch) {
          // Convert "first-last-123abc" → "First Last" (strip trailing hash)
          const slug = slugMatch[1].replace(/-[a-f0-9]{6,}$/, "");
          authorName = slug.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
        }
      }
  ...
  const extractPost = () => {
    ...
    if (isFeedPage) {
      ...
      // Try to extract engagement counts from button aria-labels
      let likeCount = 0;
